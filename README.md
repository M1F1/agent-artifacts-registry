...
The registry floor is AART `2.0.0`, raised from `1.1.1` by two facts about its own content
rather than by a routine upgrade. Its artifacts declare `requires`, a field no `1.x` parser knows
and which makes it refuse the registry outright. And `mcp/atlassian` carries a setup v2 recipe with
a package-root `SETUP.md`, which every released `1.x` rejects as `unexpected canonical package
path: SETUP.md` — the contradiction AART `2.0.0` resolves. A `1.x` client cannot read this registry
at all, so a lower floor would name executables that cannot use it.

The three AART-owned skills keep an explicit `2.0.0` minimum for the same reason and not merely to
match: they document the canonical command family, and the nine top-level verbs they replaced no
longer exist. Advisory runtime metadata still raises no floor of its own.

Registry CI installs the explicitly pinned, released AART `2.0.0` tool and runs the canonical
format, strict/frozen validate, lock, build, audit, minimum/latest compatibility, and advisory
runtime-health gates. The minimum compatibility point remains the registry's declared floor, while
`latest` and the quality tool advance deliberately; neither follows `main` or an executable patch
automatically. The committed `aart.lock.json` and `aart.index.json` are always generated by
